...
class PetsViewSet(viewsets.ModelViewSet):
    ...
    def list(self, request: HttpRequest, *args, **kwargs):
        ...
        match has_photo:
            case True:
                ...
                queryset = (
                    models.Pets.objects.exclude(photos=None)
                    .all()
                    .order_by("-created_at")
                )
            case False:
                ...
                queryset = (
                    models.Pets.objects.filter(photos=None)
                    .all()
                    .order_by("-created_at")
                )
            case _:
        ...
        if page is not None:
            ...
            count, data = len(serializer.data), serializer.data
        return Response(
            utils.pets_list_response(count, data), status=status.HTTP_200_OK
        )
    ...
    def delete(self, request, *args, **kwargs):
        if request.method == "DELETE":
            ...
            return Response(
                data=utils.pet_delete_response(deleted=len(instances), errors=errors),
    ...
    def photo(self, request: HttpRequest, pk=None):
        ...
        if request.method == "POST":
            ...
            pet.photos.add(obj)
            return Response(
                utils.pet_upload_photo_response(obj.id, obj.url),
